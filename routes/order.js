...
router.get("/searchOrders2", (req, res, next) => {
...
router.get("/searchOrders", async (req, res, next) => {
  try {
    const fYear = common.getFinancialYear(req.query.searchDate);
    const localDate = new Date(req.query.searchDate + "T00:00:00+05:30"); // Converts '2025-03-30' to local time
    ...
    // console.log(startISO, endISO, localDate);
    // ✅ Step 1: Find orders directly (Avoid `$project`, `$dateToString`, etc.)
    ...
    const transactionDetailsList = await transactionDetails
      ...
      // .select("parent_id product_id quantity")
      .lean();

    // ✅ Step 4: Merge Orders and Transaction Details
    const transactionMap = transactionDetailsList.reduce((acc, txn) => {
      if (!acc[txn.parent_id]) acc[txn.parent_id] = [];
      acc[txn.parent_id].push(txn);
      return acc;
    }, {});
...
function calculateTransactionDetails(rate_type_arr, discount_available, list) {
  ...
  if (list.details && list.details.length > 0) {
    for (let tkey in list.details) {
      let details = rate_type_arr.filter(
        (rate_type) =>
          rate_type.product.product_id == list.details[tkey].product_id
      );

      if (details.length > 0) {
        let product = details[0].product;
        //set to 'Retail' if rate is not avail
        let pt = product["rate_active"][list.details[tkey].rate_type]
          ? product["rate_active"][list.details[tkey].rate_type]
          : product["rate_active"]["Retail"];
        ...
        if (discount_available.length > 0) {
          //discount code
          let discounts = discount_available.filter(function (d) {
            return (
              d.buy_prod_id.toString() ==
                list.details[tkey].prod_id.toString() &&
              d.applicable_customer.indexOf(list.customer.customer_id) > -1 &&
              d.applicable_type.indexOf(list.details[tkey].rate_type) > -1 &&
              quantity > d.buy_count
            );
          });
          ...
          if (discounts.length > 0) {
            ...
            if (
              discounts[0].buy_prod_id.toString() ==
              discounts[0].free_prod_id.toString()
            ) {
              //only if buy and free prods are same
              // console.log("2");
...
router.post("/placeOrders", async (req, res) => {
  try {
    ...
    const localDate = new Date(req.body.orderdate + "T00:00:00+05:30"); // Converts '2025-03-30' to local time
    ...
    const [discountAvail, rateTypeArr] = await common.getRateDiscountList(reqt); // Process discounts
    // console.log(discountAvail, rateTypeArr, "tettss");
    // Process orders in parallel for discounts and transactions
    const salesDataPromises = ordersList.map((order) => {
      const newTransactionDetails = calculateTransactionDetails(
        rateTypeArr,
        discountAvail,
        order
      ...
      return {
        ...
        transactions: newTransactionDetails,
      };
    });
    ...
    const insertedSales = await sales.insertMany(
      salesData.map(({ transactions, ...sale }) => sale)
    );
    console.timeEnd("Batch Insert Sales");
    // Create a mapping of order_id → inserted sale ID
    const salesIdMap = insertedSales.reduce((acc, sale) => {
      acc[sale.sale_id] = {
        parent_id: sale._id,
        parent_date: sale.sale_date,
      };
      return acc;
    }, {});

    // Attach parent_id & parent_date to transaction details
    const updatedTransactionDetails = allTransactionDetails.map(
      (transaction) => {
        const saleInfo = salesIdMap[transaction.sale_id] || {};
        const { sale_id, ...rest } = transaction;
        return {
          ...rest,
          parent_id: saleInfo.parent_id,
          parent_date: saleInfo.parent_date,
        };
      }
    );

    console.time("Batch Insert Transactions");
    await transactionDetails.insertMany(updatedTransactionDetails);
    console.timeEnd("Batch Insert Transactions");

    console.time("Update Existing Transactions");
    // Update existing transactions in batch (avoid multiple DB calls)
    await transactionDetails.updateMany(
      {
        parent_id: { $in: ordersList.map((o) => ObjectId(o._id)) },
        type: "ORDER",
      },
      { $set: { is_delivered: true } }
    );
    console.timeEnd("Update Existing Transactions");

    console.time("Update Existing orders");
    // Update existing transactions in batch (avoid multiple DB calls)
    ...
    console.timeEnd("Update Existing orders");
...
router.post("/placeOrders2", (req, res, next) => {
  console.time("Total Execution Time");
  let _resp = {
    code: 201,
          ...
          resp.then(
            async function (result) {
              ...
              for (let key = 0; key < list.length; key++) {
                let transDetails = calculateTransactionDetails(
                  rate_type_arr,
                  discount_available,
                  list[key]
                );
                  ...
                  .then((new_sales) => {
                    ...
                    if (list[key] && list[key].details.length > 0) {
                      ...
                      for (let tkey in list[key].details) {
                        if (
                          list[key].newDetails &&
                          list[key].newDetails[tkey]
                        ) {
                          list[key].newDetails[tkey]["parent_id"] =
                            new_sales._id;
                          list[key].newDetails[tkey]["parent_date"] =
                            new_sales.sale_date;
                        }
                        ...
                        if (list[key].details[tkey].is_delivered == true) {
                          updateTransObj.push({
                            updateOne: {
                              filter: {
                                _id: ObjectId(list[key].details[tkey]._id),
                                type: "ORDER",
                              },
                      ...
                      if (list[key].discounts.length > 0) {
                        ...
                        newDiscountObj = newDiscountObj.concat(
                          list[key].discounts
                        );
                      }
                    }
                    ...
                    if (key == order_size - 1) {
                      ...
                      if (newDiscountObj.length > 0) {
                        ...
                        promises.push(
                          new Promise((resolve, reject) => {
                            discountTransaction
                              .insertMany(newDiscountObj, { ordered: false })
                              .then((sucess) => {
                                resolve(sucess);
                              })
                              .catch((error) => {
                      ...
                      if (newTransObj.length > 0) {
                        ...
                        promises.push(
                          new Promise((resolve, reject) => {
                            transactionDetails
                              .insertMany(newTransObj, { ordered: false })
                              .then((sucess) => {
                                resolve(sucess);
                              })
                              .catch((error) => {
                      ...
                      if (updateTransObj.length > 0) {
                        promises.push(
                          new Promise((resolve, reject) => {
                            transactionDetails
                              .bulkWrite(updateTransObj, { ordered: false })
                              .then((sucess) => {
                                resolve(sucess);
                              })
                              .catch((error) => {
                      ...
                      if (updateOrderObj.length > 0) {
                        promises.push(
                          new Promise((resolve, reject) => {
                            orders
                              .bulkWrite(updateOrderObj, { ordered: false })
                              .then((sucess) => {
                                resolve(sucess);
                              })
                              .catch((error) => {
  ...
  orders.countDocuments(function (err, count) {
    if (!err) {
      if (req.body.createdBy)
        req.body.createdBy = { _id: ObjectId(req.body.createdBy) };
      if (!req.body._id)
        req.body["order_id"] = common.padding(count + 1, 7, "SO");

      let newOrders = new orders(req.body);
      if (req.body._id) newOrders.isNew = false;
  ...
  orders.findByIdAndUpdate(req.params.id, { $set: req.body }, (err, orders) => {
    if (err) {
      ...
      if (!req.body.details || !req.body.details.length)
        res.json({ msg: "Orders updated successfully" });
      let count = 0;
      for (let i = 0, len = req.body.details.length; i < len; i++) {
        req.body.details[i].parent_id = orders._id;
        req.body.details[i].parent_date = orders.order_date;
