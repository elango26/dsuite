...
export class DeliveriesComponent implements OnInit {
  ...
  public addEvent() {
    let q =
      "?order_date=" +
      this.datePipe.transform(this.delDate, "yyyy-MM-dd") +
      "&route=" +
      this.selRoute;
  ...
  private loadDelivers(query: string) {
    this.commonService
      .getMethod(environment.urls.getDeliveries + query)
      .subscribe((data: any[]) => {
  ...
  public showConsolidated() {
    let q =
      ...
      this.searKey;
    this.commonService
      .getMethod(environment.urls.getConsolidatedOrderList + q)
      .subscribe((data: any[]) => {
        this.consolidatedList = data;
        ...
        const dialogRef = this.dialog.open(ConsViewComponent, {
          ...
          // height:'100%',
          data: {
            cons_data: this.consolidatedList,
            route: route,
            date: this.delDate,
          },
  ...
  applyFilter() {
    ...
    if (this.searKey != "") {
      this.tempDeliveryList = this.searDeliveryList.filter(
        (obj) =>
          obj.customer.customerName
            .toLowerCase()
            .indexOf(this.searKey.toLowerCase()) > -1
      );
    }
    this.deliveryList.next(this.tempDeliveryList);
  ...
  public editOrder(index: number, o: any) {
    ...
    const dialogRef = this.dialog.open(ProdtableComponent, {
      ...
      data: {
        ...
        order_details: o.orders,
        customer: o.customer,
        edit_details: o.orders?o.orders.details: null, // not used
        url: environment.urls.postOrder,
        isEdit: isEdit,
        source: "delivery",
      },
      panelClass: "custom-modalbox",
    ...
    dialogRef.afterClosed().subscribe((result) => {
      //reload
      console.log("edit order resutls", result);
      if (result) {
        //console.log('yes');
  ...
  public placeOrder() {
    if (
      confirm(
        "Make sure all the orders are delivered correctly, if yes click ok"
      )
    ) {
      let data = {
      ...
      this.commonService
        .postMethod(environment.urls.postOrderSales, data)
        .subscribe((data: GenericResp) => {
          if (data.code == 200) {
      ...
      this.commonService
        ...
        .subscribe((data: any[]) => {
          ...
          this.router.navigate(
            ["/layout", { outlets: { printpage: "printview" } }],
            { skipLocationChange: true }
          );
        });
    });
